...
// ====== DEPTH + HITBOX ======
const DEPTH = {
  minScale: 0.65,   // scale at top of screen (far)
  maxScale: 1.00,   // scale at bottom (near)
};
...
function scaleForY(y){
  const rect = stage.getBoundingClientRect();
  const t = Math.max(0, Math.min(1, y / Math.max(1, rect.height)));
  return DEPTH.minScale + (DEPTH.maxScale - DEPTH.minScale) * t;
}
